...
with models.DAG(
    ...
    task_set_dates_fb_login = python_operator.PythonOperator(
        task_id="task_set_dates_fb_login",
    ...
    run_fb_login = SSHOperator(
        ...
        do_xcom_push=False,
        ssh_hook=sshHook,
        command='sh /opt/dw_schibsted/yapo_bi/dw_blocketdb/run_etl_fb_login.sh -d1="{{ ti.xcom_pull(task_ids="task_set_dates_fb_login", key="start_date") }}" -d2="{{ ti.xcom_pull(task_ids="task_set_dates_fb_login", key="end_date") }}" ',  # You need add a space at the end of the command, to avoid error: Jinja template not found
    )

    task_set_dates_fb_login >> run_fb_login
