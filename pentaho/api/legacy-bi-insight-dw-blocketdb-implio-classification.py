...
with models.DAG(
    ...
    set_dates_implio_classification = python_operator.PythonOperator(
        task_id="task_set_dates_implio_classification",
    ...
    run_implio_classification = SSHOperator(
        ...
        ssh_hook=sshHook,
        command='sh /opt/dw_schibsted/yapo_bi/dw_blocketdb/implio-classification/run_implio.sh -d1="{{ ti.xcom_pull(task_ids="task_set_dates_implio_classification", key="start_date") }}" -d2="{{ ti.xcom_pull(task_ids="task_set_dates_implio_classification", key="end_date") }}" ',  # You need add a space at the end of the command, to avoid error: Jinja template not found
    )

    set_dates_implio_classification >> run_implio_classification
