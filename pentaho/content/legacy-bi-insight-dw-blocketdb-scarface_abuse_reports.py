...
with models.DAG(
    ...
    set_dates_scarface_abuse_reports = python_operator.PythonOperator(
        task_id="task_set_dates_scarface_abuse_reports",
    ...
    run_scarface_abuse_reports = SSHOperator(
        ...
        ssh_hook=sshHook,
        command='sh /opt/dw_schibsted/yapo_bi/dw_blocketdb/scarface_abuse_reports_daily_agg/run_update_scarface_abuse_reports_daily_agg.sh -d1="{{ ti.xcom_pull(task_ids="task_set_dates_scarface_abuse_reports", key="start_date") }}" -d2="{{ ti.xcom_pull(task_ids="task_set_dates_scarface_abuse_reports", key="end_date") }}" ',  # You need add a space at the end of the command, to avoid error: Jinja template not found
    )

    set_dates_scarface_abuse_reports >> run_scarface_abuse_reports
